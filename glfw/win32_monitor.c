...
monitorCallback(HMONITOR handle, HDC dc UNUSED, RECT *rect UNUSED, LPARAM data) {
    MONITORINFOEXW mi;
...
createMonitor(DISPLAY_DEVICEW *adapter, DISPLAY_DEVICEW *display) {
    ...
    if (adapter->StateFlags & DISPLAY_DEVICE_MODESPRUNED) monitor->win32.modesPruned = true;
...
_glfwSetVideoModeWin32(_GLFWmonitor *monitor, const GLFWvidmode *desired) {
    ...
    if (result == DISP_CHANGE_SUCCESSFUL) monitor->win32.modeChanged = true;
    else {
...
void
_glfwPlatformFreeMonitor(_GLFWmonitor *monitor UNUSED) {}
...
void
_glfwPlatformGetMonitorContentScale(_GLFWmonitor *monitor, float *xscale, float *yscale) {
    _glfwGetMonitorContentScaleWin32(monitor->win32.handle, xscale, yscale);
}

void
_glfwPlatformGetMonitorWorkarea(_GLFWmonitor *monitor, int *xpos, int *ypos, int *width, int *height) {
...
_glfwPlatformGetMonitorGeometry(_GLFWmonitor *monitor) {
    ...
    ZeroMemory(&mi, sizeof(mi));
    mi.cbSize = sizeof(mi);
    if (!GetMonitorInfoW(monitor->win32.handle, &mi)) return ans;
    ans.full.x = mi.rcMonitor.left;
    ans.full.y = mi.rcMonitor.top;
    ans.full.width = mi.rcMonitor.right - mi.rcMonitor.left;
    ans.full.height = mi.rcMonitor.bottom - mi.rcMonitor.top;
    ans.workarea.x = mi.rcWork.left;
    ans.workarea.y = mi.rcWork.top;
    ans.workarea.width = mi.rcWork.right - mi.rcWork.left;
    ans.workarea.height = mi.rcWork.bottom - mi.rcWork.top;
    return ans;
}
...
_glfwPlatformGetVideoMode(_GLFWmonitor *monitor, GLFWvidmode *mode) {
    ...
    if (!EnumDisplaySettingsW(monitor->win32.adapterName, ENUM_CURRENT_SETTINGS, &dm)) return false;
