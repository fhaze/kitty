...
bool
add_font_name_record(
    PyObject *table, uint16_t platform_id, uint16_t encoding_id, uint16_t language_id, uint16_t name_id, const char *string, uint16_t string_len) {
...
read_name_font_table(const uint8_t *table, size_t table_len) {
    ...
    for (; num_of_name_records > 0 && p + 6 <= (uint16_t *)slimit; num_of_name_records--) {
        ...
        if (s + length <= slimit && !add_font_name_record(ans, platform_id, encoding_id, language_id, name_id, (const char *)(s), length)) return NULL;
    }
    Py_INCREF(ans);
    return ans;
}
...
read_cv_feature_table(const uint8_t *table, const uint8_t *limit, PyObject *name_lookup_table) {
    ...
    if (limit - table >= 12) {
        ...
        if (name_id) {
            RAII_PyObject(name, get_best_name(name_lookup_table, name_id));
            if (!name) return NULL;
            if (PyDict_SetItemString(ans, "name", name) != 0) return NULL;
        }
        if (tooltip_id) {
            RAII_PyObject(tooltip, get_best_name(name_lookup_table, tooltip_id));
            if (!tooltip) return NULL;
            if (PyDict_SetItemString(ans, "tooltip", tooltip) != 0) return NULL;
        }
        if (sample_id) {
            RAII_PyObject(sample, get_best_name(name_lookup_table, sample_id));
            if (!sample) return NULL;
            if (PyDict_SetItemString(ans, "sample", sample) != 0) return NULL;
        }
        if (num_params && first_value_id) {
            RAII_PyObject(params, PyTuple_New(num_params));
            if (!params) return NULL;
            for (uint16_t i = 0; i < num_params; i++) {
                PyObject *pval = get_best_name(name_lookup_table, first_value_id + i);
                if (!pval) return NULL;
...
read_features_from_font_table(const uint8_t *table, size_t table_len, PyObject *name_lookup_table, PyObject *output) {
    ...
    for (uint16_t i = 0; i < feature_count && pos + 6 <= limit; pos += 6, i++) {
        ...
        p = (uint16_t *)(pos + 4);
        uint16_t offset_to_feature_table = next;
        const uint8_t *feature_table = feature_list_table + offset_to_feature_table;
        if (feature_table + 2 > limit) continue;
        p = (uint16_t *)(feature_table);
        uint16_t offset_to_feature_params_table = next;
...
read_STAT_font_table(const uint8_t *table, size_t table_len, PyObject *name_lookup_table, PyObject *output) {
    ...
    for (const uint8_t *pos = start_of_axis_values_offsets_array; pos + 2 <= table_limit && count < count_of_axis_value_entries; pos += 2, count++) {
        p = (uint16_t *)pos;
...
#define app(fmt, ...)                                                                                                                                        \
    {                                                                                                                                                        \
        ...
        switch (format) {
            case 1:
                if (p32 + 1 <= (uint32_t *)table_limit && axis_index < PyTuple_GET_SIZE(design_axes)) {
                    const double value = next32;
                    app("sd", "value", value);
                }
                break;
            case 2:
                if (p32 + 3 <= (uint32_t *)table_limit && axis_index < PyTuple_GET_SIZE(design_axes)) {
                    const double value = next32, minimum = next32, maximum = next32;
                    app("sd sd sd", "value", value, "minimum", minimum, "maximum", maximum);
                }
                break;
            case 3:
                if (p32 + 2 <= (uint32_t *)table_limit && axis_index < PyTuple_GET_SIZE(design_axes)) {
                    const double value = next32, linked_value = next32;
                    app("sd sd", "value", value, "linked_value", linked_value);
                }
                break;
            case 4:
                if ((uint8_t *)(p) + 6 * axis_index <= table_limit) {
                    if (num_of_multi_axis_styles >= PyTuple_GET_SIZE(multi_axis_styles)) {
                        PyErr_Format(
                            PyExc_IndexError,
                            "corrupted STAT table in font too many multi_axis_styles (%d >= %d)",
                            (int)num_of_multi_axis_styles,
                            (int)PyTuple_GET_SIZE(multi_axis_styles));
                        return false;
                    }
                    ...
                    PyObject *e = Py_BuildValue("{sH sN sO}", "flags", flags, "name", get_best_name(name_lookup_table, value_name_id), "values", values);
                    if (!e) return false;
                    PyTuple_SET_ITEM(multi_axis_styles, num_of_multi_axis_styles, e);
                    num_of_multi_axis_styles++;
                }
                break;
        }
    }
...
read_fvar_font_table(const uint8_t *table, size_t table_len, PyObject *name_lookup_table, PyObject *output) {
    ...
    const uint16_t *p = (uint16_t *)table;
    p += 2;
    const uint16_t offset_to_start_of_axis_array = next;
    next;
    ...
    if (size_of_name_record < size_of_coordinates + 4) {
        PyErr_Format(PyExc_ValueError, "size of name record: %u too small", size_of_name_record);
        return NULL;
    }
    const bool has_postscript_name = size_of_name_record >= 3 * sizeof(uint16_t) + size_of_coordinates;
    uint16_t i = 0;
    if (size_of_axis_record < 20) {
        PyErr_Format(PyExc_ValueError, "size of axis record: %u too small", size_of_axis_record);
        return NULL;
    }
    if (_PyTuple_Resize(&axes, num_of_axis_records) == -1) return NULL;
    for (const uint8_t *pos = table + offset_to_start_of_axis_array; pos + size_of_axis_record <= table + table_len && i < num_of_axis_records;
        ...
        PyObject *axis = Py_BuildValue(
            ...
            def,
            "tag",
            PyUnicode_FromStringAndSize((const char *)pos, 4),
            "hidden",
            (flags & 1) ? Py_True : Py_False,
            "strid",
            get_best_name(name_lookup_table, strid));
        if (!axis) return NULL;
    ...
    for (const uint8_t *pos = table + offset_to_start_of_axis_array + num_of_axis_records * size_of_axis_record;
         pos + size_of_name_record <= table + table_len && i < num_of_name_records;
         i++, pos += size_of_name_record) {
        p = (uint16_t *)pos;
        uint16_t name_id = next, psname_id = 0xffff;
        next;
        const uint32_t *p32 = (uint32_t *)p;
        ...
        PyObject *ns = Py_BuildValue(
            ...
            get_best_name(name_lookup_table, name_id),
            "psname",
            (psname_id != 0xffff && psname_id ? get_best_name(name_lookup_table, psname_id) : PyUnicode_FromString("")));
        if (!ns) return NULL;
