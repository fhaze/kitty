...
typedef struct {
    PyObject_HEAD ShlexState state;
    PyObject *src;
    bool yielded;
    void *data;
    int kind;
...
dealloc(Shlex *self) {
    Py_CLEAR(self->src);
    dealloc_shlex_state(&self->state);
}
...
next(PyObject *self_) {
    ...
    switch (len) {
        ...
        case -2:
            ...
            len = 0;
            /* fallthrough */
        default: self->yielded = true; return PyUnicode_FromStringAndSize(self->state.buf, (Py_ssize_t)len);
    }
}
...
static PyMethodDef methods[] = {
    {"next_word", (PyCFunction)next_word_with_position, METH_NOARGS, ""}, {NULL} /* Sentinel */
};

PyTypeObject Shlex_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.Shlex",
