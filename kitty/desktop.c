...
#undef F
    ...
    if (!sn_display) {
        PyErr_SetString(PyExc_OSError, "Failed to create SnDisplay");
        return NULL;
    }
    ...
    if (!ctx) {
        PyErr_SetString(PyExc_OSError, "Failed to create startup-notification context");
        return NULL;
    }
...
typedef int (*ca_context_play_func)(void *, uint32_t, ...);
static ca_context_play_func ca_context_play = NULL;
typedef int (*ca_context_change_props_func)(void *, ...);
static ca_context_change_props_func ca_context_change_props = NULL;
...
load_libcanberra(void) {
    ...
    if (PyErr_Occurred()) {
        PyErr_Print();
        dlclose(libcanberra_handle);
        libcanberra_handle = NULL;
        return;
    }
...
play_canberra_sound(const char *which_sound, const char *event_id, bool is_path, const char *media_role, const char *theme_name) {
    ...
    if (canberra_pipe_r == -1) {
        ...
        canberra_pipe_r = fds[0];
        canberra_pipe_w = fds[1];
