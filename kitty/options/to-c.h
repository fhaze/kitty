...
color_as_int(PyObject *color) {
    if (!PyObject_TypeCheck(color, &Color_Type)) {
        PyErr_SetString(PyExc_TypeError, "Not a Color object");
        return 0;
    }
    Color *c = (Color *)color;
...
background_images(PyObject *src, Options *opts) {
    ...
    if (new_count == opts->background_images.count && opts->background_images.paths) {
        ...
        for (size_t i = 0; i < new_count; i++) {
            ...
            if (!p || !opts->background_images.paths[i] || strcmp(p, opts->background_images.paths[i]) != 0) {
                changed = true;
                break;
            }
        }
    ...
    if (opts->background_images.paths) {
        ...
        for (size_t i = 0; i < opts->background_images.count; i++) { opts->background_images.paths[i] = strdup(PyUnicode_AsUTF8(PyTuple_GET_ITEM(src, i))); }
    }
}

static inline void
bell_path(PyObject *src, Options *opts) {
    STR_SETTER(bell_path);
}

static inline void
bell_theme(PyObject *src, Options *opts) {
    STR_SETTER(bell_theme);
}

static inline void
macos_ns_window_layer(PyObject *src, Options *opts) {
    STR_SETTER(macos_ns_window_layer);
}

static inline void
window_logo_path(PyObject *src, Options *opts) {
    STR_SETTER(default_window_logo);
}
...
#define S(which)                                                            \
    ...
    S(strikethrough_thickness);
    S(strikethrough_position);
    S(cell_height);
    S(cell_width);
    S(baseline);
#undef S
...
font_features(PyObject *mf, Options *opts) {
    ...
    while (PyDict_Next(mf, &pos, &key, &value)) {
        ...
        Py_ssize_t psname_sz;
        const char *psname = PyUnicode_AsUTF8AndSize(key, &psname_sz);
...
url_prefixes(PyObject *up, Options *opts) {
    if (!PyTuple_Check(up)) {
        PyErr_SetString(PyExc_TypeError, "url_prefixes must be a tuple");
        return;
    }
    ...
    for (size_t i = 0; i < opts->url_prefixes.num; i++) {
        ...
        if (!PyUnicode_Check(t)) {
            PyErr_SetString(PyExc_TypeError, "url_prefixes must be strings");
            return;
        }
        ...
        for (size_t x = 0; x < opts->url_prefixes.values[i].len; x++) { opts->url_prefixes.values[i].string[x] = PyUnicode_READ(kind, PyUnicode_DATA(t), x); }
    }
}
...
free_menu_map(Options *opts) {
    if (opts->global_menu.entries) {
        ...
        free(opts->global_menu.entries);
        opts->global_menu.entries = NULL;
    }
...
menu_map(PyObject *entry_dict, Options *opts) {
    if (!PyDict_Check(entry_dict)) {
        PyErr_SetString(PyExc_TypeError, "menu_map entries must be a dict");
        return;
    }
    free_menu_map(opts);
    ...
    while (PyDict_Next(entry_dict, &pos, &key, &value)) {
        if (PyTuple_Check(key) && PyTuple_GET_SIZE(key) > 1 && PyUnicode_Check(value) &&
            PyUnicode_CompareWithASCIIString(PyTuple_GET_ITEM(key, 0), "global") == 0) {
...
underline_exclusion(PyObject *val, Options *opts) {
    if (!PyTuple_Check(val)) {
        PyErr_SetString(PyExc_TypeError, "underline_exclusion must be a tuple");
        return;
    }
...
text_composition_strategy(PyObject *val, Options *opts) {
    if (!PyUnicode_Check(val)) {
        PyErr_SetString(PyExc_TypeError, "text_composition_strategy must be a string");
        return;
    }
    opts->text_old_gamma = false;
    opts->text_gamma_adjustment = 1.0f;
    opts->text_contrast = 0.f;
    if (PyUnicode_CompareWithASCIIString(val, "platform") == 0) {
#ifdef __APPLE__
        opts->text_gamma_adjustment = 1.7f;
        opts->text_contrast = 30.f;
#endif
        ...
        if (size < 1 || 2 < size) {
            PyErr_SetString(PyExc_ValueError, "text_composition_strategy must be of the form 'number [number]'");
            return;
        }
...
list_of_chars(PyObject *chars) {
    if (!PyUnicode_Check(chars)) {
        PyErr_SetString(PyExc_TypeError, "list_of_chars must be a string");
        return NULL;
    }
    char_type *ans = calloc(PyUnicode_GET_LENGTH(chars) + 1, sizeof(char_type));
    if (ans) {
        for (ssize_t i = 0; i < PyUnicode_GET_LENGTH(chars); i++) { ans[i] = PyUnicode_READ(PyUnicode_KIND(chars), PyUnicode_DATA(chars), i); }
    }
    return ans;
}
...
focus_follows_mouse(PyObject *val, Options *opts) {
    ...
    switch (q[0]) {
        ...
        case 't':
            ...
            break;
        case 'd': opts->focus_follows_mouse.on_drop = true; break;
    }
}
...
#define F(x)       \
    ...
    F(select_by_word_characters_forward);
    F(bell_path);
    F(bell_theme);
    F(macos_ns_window_layer);
