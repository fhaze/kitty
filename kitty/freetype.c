...
typedef struct FaceMetrics {
    ...
    int ascender, descender, height, max_advance_width, max_advance_height, underline_position, underline_thickness, strikethrough_position,
...
#define CPY(n) self->metrics.n = self->face->n;
    ...
    CPY(descender);
    CPY(height);
    CPY(max_advance_width);
    CPY(max_advance_height);
    CPY(underline_position);
    CPY(underline_thickness);
#undef CPY
}
...
face_equals_descriptor(PyObject *face_, PyObject *descriptor) {
    ...
    FT_Matrix dmat = {0};
    bool d_has = false;
    if (!read_matrix_from_descriptor(descriptor, &dmat, &d_has)) {
        PyErr_Clear();
        return false;
    }
    if (d_has != face->has_matrix) return false;
    if (d_has && (face->matrix.xx != dmat.xx || face->matrix.xy != dmat.xy || face->matrix.yx != dmat.yx || face->matrix.yy != dmat.yy)) return false;
    return true;
}
...
#undef D
    ...
    if (retval != NULL) {
        int error;
        if ((error = FT_New_Face(library, path, index, &(self->face)))) {
            self->face = NULL;
            return set_load_error(path, error);
        }
        if (!init_ft_face(self, PyDict_GetItemString(descriptor, "path"), hinting, hint_style, index, fg)) {
            Py_CLEAR(retval);
            return NULL;
        }
        ...
        if (!create_features_for_face(postscript_name_for_face((PyObject *)self), PyDict_GetItemString(descriptor, "features"), &self->font_features))
            return NULL;
...
FontFeatures *
features_for_face(PyObject *s) {
    return &((Face *)s)->font_features;
}
...
static inline void
cleanup_ftmm(FT_MM_Var **p) {
    if (*p) FT_Done_MM_Var(library, *p);
    *p = NULL;
}
...
#define B(x) ((x) ? Py_True : Py_False)
    ...
    return PyUnicode_FromFormat(
        ...
        self->face->family_name ? self->face->family_name : "",
        self->face->style_name ? self->face->style_name : "",
        ps_name ? ps_name : "",
        self->path,
        instance.ttc_index,
        B(FT_IS_VARIATION(self->face)),
        B(FT_IS_NAMED_INSTANCE(self->face)),
        B(self->is_scalable),
        B(self->has_color));
#undef B
}
...
calc_cell_width(Face *self) {
    ...
    for (char_type i = 32; i < 128; i++) {
        ...
        if (load_glyph(self, glyph_index, FT_LOAD_DEFAULT)) { ans = MAX(ans, (unsigned int)ceilf((float)self->face->glyph->metrics.horiAdvance / 64.f)); }
    }
...
is_colrv0_glyph(Face *self, int glyph_id) {
    ...
    FT_UInt layer_glyph_index = 0, layer_color_index = 0;
...
is_glyph_empty(PyObject *s, glyph_index g) {
    Face *self = (Face *)s;
    if (!load_glyph(self, g, FT_LOAD_DEFAULT)) {
        PyErr_Print();
        return false;
    }
...
hb_font_t *
harfbuzz_font_for_face(PyObject *self) {
    return ((Face *)self)->harfbuzz_font;
}
...
populate_processed_bitmap(FT_GlyphSlotRec *slot, FT_Bitmap *bitmap, ProcessedBitmap *ans, bool copy_buf) {
    ...
    ans->start_x = 0;
    ans->width = bitmap->width;
    ans->pixel_mode = bitmap->pixel_mode;
    ans->bitmap_top = slot->bitmap_top;
    ans->bitmap_left = slot->bitmap_left;
}
...
freetype_convert_mono_bitmap(FT_Bitmap *src, FT_Bitmap *dest) {
    ...
    if (error) {
        set_freetype_error("Failed to convert bitmap, with error:", error);
        return false;
    }
...
render_bitmap(
    ...
    if (ans->width > max_width) {
        ...
        if (italic && extra < cell_width / 2) {
            ...
            if (set_font_size(
                    self,
                    (FT_F26Dot6)((float)self->char_width * ar),
                    (FT_F26Dot6)((float)self->char_height * ar),
                    self->xdpi,
                    self->ydpi,
                    0,
                    fg->fcm.cell_height)) {
...
get_preferred_palette_index(Face *self) {
    if (!self->palettes_scanned) {
        ...
        self->dark_palette_index = CAIRO_COLOR_PALETTE_DEFAULT;
        self->light_palette_index = CAIRO_COLOR_PALETTE_DEFAULT;
...
ensure_cairo_resources(Face *self, size_t width, size_t height) {
    ...
    if (stride * height > self->cairo.stride * self->cairo.height) {
        ...
        self->cairo.width = width;
        self->cairo.height = height;
        self->cairo.size_in_px = 0;
...
#define check(msg)                                                       \
    if ((s = cairo_font_options_status(opts)) != CAIRO_STATUS_SUCCESS) { \
        ...
        check("Failed to set cairo hintstyle");
        cairo_font_options_set_color_palette(opts, get_preferred_palette_index(self));
        check("Failed to set cairo palette index");
        set_variation_for_cairo(self, opts);
        check("Failed to set cairo font variations");
...
render_glyph_with_cairo(Face *self, int glyph_id, ProcessedBitmap *ans, unsigned width, unsigned height, ARGB32 fg, unsigned cell_baseline) {
    ...
    if (!width || !height) {
        ...
        width = (unsigned)ceil(fm.max_x_advance);
        height = (unsigned)ceil(fm.height);
    ...
    ans->buf = self->cairo.buf;
    ans->needs_free = false;
    ans->start_x = 0;
    ans->width = width;
...
static bool
render_color_bitmap(
    Face *self, int glyph_id, ProcessedBitmap *ans, unsigned int cell_width, unsigned int cell_height, unsigned int num_cells, unsigned int baseline) {
...
ensure_name_table(Face *self) {
    ...
    for (FT_UInt i = 0; i < FT_Get_Sfnt_Name_Count(self->face); i++) {
        ...
        if (err != 0) continue;
        if (!add_font_name_record(ans, temp.platform_id, temp.encoding_id, temp.language_id, temp.name_id, (const char *)temp.string, temp.string_len))
            return NULL;
    }
    self->name_lookup_table = ans;
    Py_INCREF(ans);
    return true;
}
...
convert_named_style_to_python(Face *face, const FT_Var_Named_Style *src, FT_Var_Axis *axes, unsigned num_of_axes) {
    ...
    return Py_BuildValue(
        "{sO sO sO}", "axis_values", axis_values, "name", name ? name : PyUnicode_FromString(""), "psname", psname ? psname : PyUnicode_FromString(""));
}
...
convert_axis_to_python(Face *face, const FT_Var_Axis *src, FT_UInt flags) {
    ...
    if (!strid) {
        PyErr_Clear();
        strid = PyUnicode_FromString("");
    }
    uint8_t tag_buf[5] = {0};
    return Py_BuildValue(
        "{sd sd sd sO ss ss sN}",
        "minimum",
        src->minimum / 65536.0,
        "maximum",
        src->maximum / 65536.0,
        ...
        src->def / 65536.0,
        "hidden",
        flags & FT_VAR_AXIS_FLAG_HIDDEN ? Py_True : Py_False,
        "name",
        src->name,
        "tag",
        tag_to_string(src->tag, tag_buf),
        "strid",
        strid);
}
...
get_variation(Face *self, PyObject *a UNUSED) {
    ...
    if ((err = FT_Get_Var_Design_Coordinates(self->face, mm->num_axis, coords))) {
        set_freetype_error("Failed to load the variation data from font with error:", err);
        return NULL;
    }
    RAII_PyObject(ans, PyDict_New());
    if (!ans) return NULL;
...
get_features(Face *self, PyObject *a UNUSED) {
    ...
    if ((err = FT_Load_Sfnt_Table(self->face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0, NULL, &length)) == 0) {
        ...
        if ((err = FT_Load_Sfnt_Table(self->face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0, table, &length))) {
            set_freetype_error("Failed to load the GSUB table from font with error:", err);
            return NULL;
        }
...
get_variable_data(Face *self, PyObject *a UNUSED) {
    ...
    if ((err = FT_Load_Sfnt_Table(self->face, FT_MAKE_TAG('S', 'T', 'A', 'T'), 0, NULL, &length)) == 0) {
        ...
        if ((err = FT_Load_Sfnt_Table(self->face, FT_MAKE_TAG('S', 'T', 'A', 'T'), 0, table, &length))) {
            set_freetype_error("Failed to load the STAT table from font with error:", err);
            return NULL;
        }
    ...
    if (self->is_variable) {
        RAII_FTMMVar(mm);
        if ((err = FT_Get_MM_Var(self->face, &mm))) {
            set_freetype_error("Failed to get variable axis data from font with error:", err);
            return NULL;
        }
...
static void
destroy_hb_buffer(hb_buffer_t **x) {
    if (*x) hb_buffer_destroy(*x);
}
...
render_sample_text(Face *self, PyObject *args) {
    ...
    if (!hb_buffer_pre_allocate(hb_buffer, 4 * PyUnicode_GET_LENGTH(ptext))) {
        PyErr_NoMemory();
        return NULL;
    }
...
PyTypeObject Face_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.Face",
    .tp_new = new,
